...
export async function POST(request: Request) {
  ...
  if (!destination) {
    return NextResponse.json({ error: "DESTINATION_MISSING" }, { status: 400 });
  }

  if (process.env.NODE_ENV === "production" && process.env.ONBOARDING_VERIFICATION_MODE !== "external") {
    return NextResponse.json(
      { error: "VERIFICATION_PROVIDER_NOT_CONFIGURED" },
      { status: 503 },
    );
  }
  ...
  await prisma.onboardingVerificationChallenge.create({
    data: {
      ...
      channel,
      codeHash: hashVerificationCode(session.user.id, channel, code),
      expiresAt,
    },
  });

  // Local/preview QA only. Production never exposes OTP.
  return NextResponse.json({
    ok: true,
